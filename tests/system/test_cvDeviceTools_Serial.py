...
class TestCvDeviceToolsWithSerial():
    ...
    def test_get_device_facts(self, CV_DEVICE):
        ...
        if FIELD_SERIAL in CV_DEVICE:
            assert self.inventory.get_device_facts(device_lookup=CV_DEVICE[FIELD_SERIAL])
        else:
    ...
    def test_get_device_id(self, CV_DEVICE):
        ...
        logging.debug("CV_DEVICE data is: %s", str(CV_DEVICE))
        if FIELD_SERIAL in CV_DEVICE:
            assert self.inventory.get_device_id(device_lookup=CV_DEVICE[FIELD_SERIAL]) == CV_DEVICE[FIELD_SYSMAC]
        else:
            logging.info('Device not based on serial number')
        logging.info('End of CV query at {}'.format(time_log()))
    ...
    def test_get_device_id(self, CV_DEVICE):
        ...
        logging.debug("CV_DEVICE data is: %s", str(CV_DEVICE))
        if FIELD_SERIAL in CV_DEVICE:
            cv_data = self.inventory.get_device_configlets(device_lookup=CV_DEVICE[FIELD_SERIAL])
    ...
    def test_get_device_container(self, CV_DEVICE):
        ...
        logging.debug("CV_DEVICE data is: %s", str(CV_DEVICE))
        if FIELD_SERIAL in CV_DEVICE:
            assert self.inventory.get_device_container(device_lookup=CV_DEVICE[FIELD_SERIAL])[FIELD_PARENT_NAME] == CV_DEVICE[FIELD_PARENT_NAME]
        else:
            logging.info('Device not based on serial number')
        logging.info('End of CV query at {}'.format(time_log()))
    ...
    def test_device_is_present_by_serial(self, CV_DEVICE):
        ...
        logging.info('Start CV query at {}'.format(time_log()))
        if FIELD_SERIAL in CV_DEVICE:
            assert self.inventory.is_device_exist(device_lookup=CV_DEVICE[FIELD_SERIAL]) is True
            logging.info('End of CV query at {}'.format(time_log()))
            logging.info('Device {} is not present on Cloudvision'.format(CV_DEVICE[FIELD_SERIAL]))
        else:
    ...
    def test_device_in_container(self, CV_DEVICE):
        ...
        if FIELD_SERIAL in CV_DEVICE:
            ...
            logging.info('Device {} is correctly configured under {}'.format(CV_DEVICE[FIELD_SERIAL], CV_DEVICE[FIELD_PARENT_NAME]))
        else:
            logging.info('Device not based on serial number')
        logging.info('End of CV query at {}'.format(time_log()))
